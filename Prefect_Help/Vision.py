...
def debug_imshow(image, idx=0) -> None:
    if _DEBUG:
        if image is None:
            return
        caller = inspect.stack()[1][3]
        name = f"{caller}_{idx}" if idx != 0 else f"{caller}"
        if _DEBUG_SAVEIMG:
            if not os.path.exists("vision_debug"):
                os.mkdir("vision_debug")
            cv2.imwrite(f"vision_debug/{name}.jpg", image)
        else:
...
def find_laser_point(img) -> Tuple[bool, float, float]:
    ...
    mask = cv2.inRange(hsv, RED_LOWER, RED_UPPER) + cv2.inRange(hsv, RED_LOWER2, RED_UPPER2)
    # 把红色的区域变成白色，其他区域变成黑色
    ...
    if _DEBUG:
        ...
        debug_imshow(img_with_keypoints, 2)
    if keypoints:
...
def find_red_area(img) -> Tuple[bool, float, float, float]:
    ...
    filtered_contours = [cnt for area, cnt in sorted_contours if area >= MIN_AERA]
    filtered_areas = [area for area, cnt in sorted_contours if area >= MIN_AERA]
    if _DEBUG:
...
def find_QRcode_contour(frame) -> Tuple[bool, float, float]:
    ...
    # 提取图像梯度提取二维码区域
    gradX = cv2.Sobel(image, ddepth=cv2.CV_32F, dx=1, dy=0, ksize=-1)  # type: ignore
    gradY = cv2.Sobel(image, ddepth=cv2.CV_32F, dx=0, dy=1, ksize=-1)  # type: ignore
...
def detect_apriltag(
    ...
    if camera_params is not None and tag_size is not None:
        results = _ap_detector.detect(gray, estimate_tag_pose=True, camera_params=camera_params, tag_size=tag_size)
    else:
